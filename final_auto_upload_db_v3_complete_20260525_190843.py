...
def load_case_data(case_no):
    ...
    if properties:
        ...
        case['property_bank_name'] = prop.get('bank_name', '')
        case['property_bank_account'] = prop.get('bank_account', '')
        case['property_amount'] = prop.get('amount', '')
        case['property_currency'] = prop.get('currency', '')
...
def add_applicant(page1):
    ...
    if applicant_type == '自然人':
        ...
        print(f"选择性别: {gender}...")
        # 兼容"男"和"男性"
        gender_short = gender.replace('性', '')
        try:
            page1.get_by_role("radio", name=gender, exact=True).click()
        except:
            try:
                page1.get_by_role("radio", name=gender_short, exact=True).click()
            except:
                # 使用JavaScript点击
                page1.evaluate(f"() => {{ const radios = document.querySelectorAll('input[type=radio]'); for(let r of radios) {{ if(r.value === '{gender}' || r.value === '{gender_short}') {{ r.click(); return; }} }} }}")
    ...
    print("保存申请人...")
    page1.locator("#addSQR").get_by_role("button", name="保存").click()
    time.sleep(2)
...
def add_property(page1):
    ...
    if property_type == '存款':
        ...
        province = CASE_DATA.get('property_province', '')
        if province:
            print(f"选择开户行所在地: {province}...")
            try:
                # 方法1：通过placeholder定位
                try:
                    page1.get_by_placeholder("选择开户行所在地").click()
                except:
                    # 方法2：通过label文本定位
                    page1.locator(".el-form-item").filter(has_text="开户行所在地").locator(".el-input__inner").click()
                time.sleep(1)
                # 选择选项
                try:
                    page1.get_by_text(province, exact=True).first.click()
                except:
                    page1.get_by_text(province, exact=False).first.click()
                print("  选择成功")
            except Exception as e:
                print(f"  选择失败: {e}")
            time.sleep(0.5)
        ...
        # 输入开户银行名称
        bank = CASE_DATA.get('property_bank_name', '') or CASE_DATA.get('bank_name', '')
        if bank:
            print(f"输入开户银行名称: {bank}...")
            try:
                bank_input = page1.locator(".el-form-item").filter(has_text="开户银行名称").locator(".el-input__inner")
                bank_input.click()
        ...
        # 输入开户账号
        account = CASE_DATA.get('property_bank_account', '') or CASE_DATA.get('bank_account', '')
        if account:
            print(f"输入开户账号: {account}...")
            try:
                account_input = page1.locator(".el-form-item").filter(has_text="开户账号").locator(".el-input__inner")
                account_input.click()
        ...
        # 输入数额
        amount = str(CASE_DATA.get('property_amount', CASE_DATA.get('preserve_amount', 0)))
        print(f"输入数额: {amount}...")
        try:
            amount_input = page1.locator(".el-form-item").filter(has_text="数额").locator(".el-input__inner")
            amount_input.click()
            amount_input.fill(amount)
            print("  成功")
        except Exception as e:
        ...
        # 选择币种
        currency = CASE_DATA.get('property_currency', 'CNY')
        currency_map = {'CNY': '人民币', 'USD': '美元', 'EUR': '欧元'}
        currency_text = currency_map.get(currency, currency)
        print(f"选择币种: {currency_text}...")
        try:
            page1.get_by_placeholder("请选择单位").click()
            time.sleep(1)
            page1.get_by_text(currency_text, exact=True).click()
            print("  选择成功")
        except Exception as e:
            print(f"  选择失败: {e}")
        time.sleep(0.5)
...
def add_guarantee(page1):
    ...
    # 提供保证人方式必须填写担保物名称，默认使用申请人姓名+信用担保
    if not guarantee_object and guarantee_type == '提供保证人':
        guarantee_object = CASE_DATA.get('applicant_name', '') + '信用担保'
    if guarantee_object:
...
def upload_files(page1):
    ...
    for i, file_input in enumerate(file_inputs):
        try:    # 获取input的父元素，确定材料类型
                ...
                print(f"  文件input {i+1}: {parent_text[:50]}")
                ...
                if '保全申请书' in parent_text:
                    ...
                    if '身份证明材料' in file_map:
                        for f in file_map['身份证明材料']:
                            if '被申请人' in f['name']:
                                files_to_upload.append(f['path'])
                elif '担保' in parent_text or '保证人' in parent_text or '保证金' in parent_text:
                    if '担保材料' in file_map:
