...
找到 {len(file_inputs)} 个文件上传input")
            ...
            # 遍历每个材料类别
            for header, folder in header_map.items():
...
【{header}】找到 {len(files)} 个文件")
                ...
                for fname in files:
                    ...
                    try:
                        # 找到包含表头的元素
                        header_els = page1.locator(f"text={header}").all()
                        for hel in header_els:
                            if not hel.is_visible():
                                continue
                            
                            # 向上查找父元素，然后找input
                            parent = hel.locator("xpath=..")
                            inp = parent.locator("input[type='file']").first
                            
                            if inp.count() > 0:
                                inp.set_input_files(fpath)
                                print(f"    ✓ 成功")
                                time.sleep(2)
                                break
                            else:
                                # 尝试点击上传按钮触发input
                                btn = parent.locator("button, text=/上传/i").first
                                if btn.is_visible():
                                    btn.click()
                                    time.sleep(1)
                                    # 重新查找input
                                    new_inps = page1.locator("input[type='file']").all()
                                    for ni in new_inps:
                                        try:
                                            ni.set_input_files(fpath)
                                            print(f"    ✓ 成功")
                                            time.sleep(2)
                                            break
                                        except:
