    ...
    try:
        result = page1.evaluate("""() => {
            ...
            for(let item of formItems) {
                ...
                const uploadBtn = item.querySelector('button, .el-button, .upload-btn, [class*=upload]');
                if(label && uploadBtn) {
                    areas.push({
        ...
        print(f"  找到 {len(result)} 个上传区域")
        for area in result:
            print(f"    - {area.get('label', 'unknown')}")
    except Exception as e:
        print(f"  JS查找失败: {e}")
    
    # 方法2：使用Playwright查找所有包含"上传"文本的元素
    try:
        # 查找所有包含"上传"的按钮
        all_buttons = page1.locator("button, .el-button, div[role=button]").all()
        upload_buttons = []
        for btn in all_buttons:
            try:
                text = btn.text_content()
                if text and '上传' in text:
        ...
        for i, btn in enumerate(upload_buttons):
            try:
                ...
                label = None
                try:
                    label = btn.locator("xpath=ancestor::div[contains(@class, 'el-form-item')]//label").text_content()
                except:
                    pass
                ...
                if not label:
                    # 尝试通过兄弟元素查找标签
                    try:
                        label = btn.locator("xpath=preceding-sibling::label").text_content()
                    except:
                        pass
