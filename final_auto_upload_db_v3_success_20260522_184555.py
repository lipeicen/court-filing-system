...
def add_applicant(page1):
    ...
    if applicant_type == '自然人':
        ...
        name = CASE_DATA.get('applicant_name', '')
        print(f"输入单位名称: {name}...")
        page1.evaluate(f"() => {{ const inputs = document.querySelectorAll('input'); for(let input of inputs) {{ if(input.placeholder && input.placeholder.includes('单位名称')) {{ input.focus(); input.value = '{name}'; input.dispatchEvent(new Event('input', {{bubbles:true}})); return; }} }} }}")
        time.sleep(0.5)
        
        legal = CASE_DATA.get('applicant_legal_person', '')
        print("输入法定代表人...")
        page1.evaluate(f"() => {{ const inputs = document.querySelectorAll('input'); for(let input of inputs) {{ if(input.placeholder && input.placeholder.includes('法定代表人')) {{ input.focus(); input.value = '{legal}'; input.dispatchEvent(new Event('input', {{bubbles:true}})); return; }} }} }}")
        time.sleep(0.5)
        
        credit = CASE_DATA.get('applicant_uscc', '')
        print("输入统一社会信用代码...")
        page1.evaluate(f"() => {{ const inputs = document.querySelectorAll('input'); for(let input of inputs) {{ if(input.placeholder && input.placeholder.includes('信用代码')) {{ input.focus(); input.value = '{credit}'; input.dispatchEvent(new Event('input', {{bubbles:true}})); return; }} }} }}")
        time.sleep(0.5)
        
        phone = CASE_DATA.get('applicant_phone', '')
        print("输入手机号...")
        page1.evaluate(f"() => {{ const inputs = document.querySelectorAll('input'); for(let input of inputs) {{ if(input.placeholder && input.placeholder.includes('手机号码')) {{ input.focus(); input.value = '{phone}'; input.dispatchEvent(new Event('input', {{bubbles:true}})); return; }} }} }}")
        time.sleep(0.5)
        ...
        name = CASE_DATA.get('applicant_name', '')
        print(f"输入单位名称: {name}...")
        page1.evaluate("() => { const inputs = document.querySelectorAll('input'); for(let input of inputs) { if(input.placeholder && input.placeholder.includes('单位名称')) { input.focus(); input.value = '" + name + "'; input.dispatchEvent(new Event('input', {bubbles:true})); return; } } }")
        time.sleep(0.5)
        
        legal = CASE_DATA.get('applicant_legal_person', '')
        print("输入法定代表人...")
        page1.evaluate("() => { const inputs = document.querySelectorAll('input'); for(let input of inputs) { if(input.placeholder && input.placeholder.includes('法定代表人')) { input.focus(); input.value = '" + legal + "'; input.dispatchEvent(new Event('input', {bubbles:true})); return; } } }")
        time.sleep(0.5)
        
        credit = CASE_DATA.get('applicant_uscc', '')
        print("输入统一社会信用代码...")
        page1.evaluate("() => { const inputs = document.querySelectorAll('input'); for(let input of inputs) { if(input.placeholder && input.placeholder.includes('信用代码')) { input.focus(); input.value = '" + credit + "'; input.dispatchEvent(new Event('input', {bubbles:true})); return; } } }")
        time.sleep(0.5)
        
        phone = CASE_DATA.get('applicant_phone', '')
        print("输入手机号...")
        page1.evaluate("() => { const inputs = document.querySelectorAll('input'); for(let input of inputs) { if(input.placeholder && input.placeholder.includes('手机号码')) { input.focus(); input.value = '" + phone + "'; input.dispatchEvent(new Event('input', {bubbles:true})); return; } } }")
        time.sleep(0.5)
...
def add_respondent(page1):
    ...
    if respondent_type == '自然人':
        ...
        print(f"选择性别: {gender}...")
        page1.locator("#addBSQR").get_by_role("radio", name=gender).click()
        time.sleep(0.5)
...
def add_property(page1):
    ...
    try:
        ...
        try:
            ...
            # 方法3：直接用JavaScript查找财产线索区域并点击添加
    ...
    if not clicked:
        try:
            page1.evaluate(f"() => {{ const all = document.querySelectorAll('*'); for(let el of all) {{ if(el.children.length === 0 && el.textContent.trim() === '{property_type}') {{ el.click(); return true; }} }} return false; }}")
            clicked = True
            print("  方法3成功")
        except:
            pass
    ...
    # 方法2：查找所有input，找到财产所有人相关的
    if not clicked:
        try:
            page1.evaluate("() => { const inputs = document.querySelectorAll('input'); for(let input of inputs) { if(input.placeholder && input.placeholder.includes('财产所有人')) { input.click(); return true; } } return false; }")
            time.sleep(1)
            page1.evaluate(f"() => {{ const items = document.querySelectorAll('li, div, span'); for(let item of items) {{ if(item.textContent.includes('{owner}')) {{ item.click(); return true; }} }} return false; }}")
            clicked = True
            print("  方法2成功")
        except:
            pass
    ...
    if not clicked:
        try:
            page1.evaluate(f"() => {{ const inputs = document.querySelectorAll('input'); for(let input of inputs) {{ let p = input.parentElement; for(let i=0; i<5; i++) {{ if(!p) break; if(p.textContent.includes('财产所有人')) {{ input.value = '{owner}'; input.dispatchEvent(new Event('input', {{bubbles:true}})); return true; }} p = p.parentElement; }} }} return false; }}")
            clicked = True
            print("  方法3成功")
        except:
            pass
    ...
    if property_type == '房产':
        ...
        if province:
            try:
                ...
                time.sleep(1)
                # 使用JavaScript选择省份
                page1.evaluate(f"() => {{ const items = document.querySelectorAll('li, div, span'); for(let item of items) {{ if(item.textContent.trim() === '{province}') {{ item.click(); return true; }} }} return false; }}")
                time.sleep(0.5)
                print("  省份选择成功")
    ...
    try:
        # 在财产线索弹窗内查找保存按钮
        page1.locator("#addCCXS").get_by_role("button", name="保存", exact=True).click()
    except:
        try:
            # 备用：查找最后一个保存按钮
            buttons = page1.get_by_role("button", name="保存", exact=True).all()
            if len(buttons) > 0:
                buttons[-1].click()
            else:
