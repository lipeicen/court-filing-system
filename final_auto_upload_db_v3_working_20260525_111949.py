...
def add_property(page1):
    ...
    if property_type == '房产':
        ...
        if province:
            ...
            try:
                # 找到房产坐落位置的下拉框（第一个）
                location_dropdown = page1.locator(".el-form-item").filter(has_text="房产坐落位置").locator(".el-input__inner").first
                ...
                try:
                    # 先截图查看下拉框状态
                    page1.screenshot(path=r'C:\court-auto-filing\debug_province_dropdown.png')
                    ...
                    print(f"  省份选择成功: {province}")
                except:
                    try:
                        # 备用：使用JavaScript点击（先滚动到顶部，再查找）
                        page1.evaluate(f"""() => {{
                            // 先找到下拉列表容器并滚动到顶部
                            const dropdown = document.querySelector('.el-select-dropdown .el-scrollbar__wrap, .el-select-dropdown__wrap, .uni-scroll-view');
                            if(dropdown) {{
                            ...
                            // 等待一下然后查找
                            setTimeout(() => {{
                                const items = document.querySelectorAll('li, .el-select-dropdown__item, .el-dropdown-menu__item');
                                for(let item of items) {{
                                    if(item.textContent.trim() === '{province}') {{
                                        item.click();
                                    }}
                                }}
                            }}, 500);
...
def upload_files(page1):
    ...
    # 如果找到文件input，直接使用Playwright的set_input_files
    if len(file_inputs) > 0:
        ...
        for i, file_input in enumerate(file_inputs):
            try:
                # 获取该input对应的材料类型标签
                label = file_input.locator("xpath=ancestor::div[contains(@class, 'el-form-item')]//label").text_content()
